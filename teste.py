...
from database import get_dados_dashboard
import json
import os
...
if not st.session_state.logged_in:
    ...
    if menu == "Gerenciar Usuários" and st.session_state.role == 'admin':
        ...
        col1, col2 = st.columns(2)
        with col1:
            mes = st.selectbox("Mês", range(1, 13), format_func=lambda x: datetime(2000, x, 1).strftime('%B'))
            print(mes)
        with col2:
            ano = st.selectbox("Ano", range(2020, datetime.now().year + 1), index=len(range(2020, datetime.now().year + 1))-1)
            print(ano)

        # Buscar dados do banco
        try:
            dados = get_dados_dashboard(mes, ano, st.session_state.nome)
            ...
            st.dataframe(tabela_detalhada.style.format({
